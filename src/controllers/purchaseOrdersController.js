...
exports.create = async (req, res, next) => {
  try {
    ...
    const respondWithPurchase = async (poDoc) => {
      ...
      // Auto-sync inventory when purchase order is marked as completed
      if (data.status === 'completed' && Array.isArray(data.items) && data.items.length > 0) {
        try {
...
exports.update = async (req, res, next) => {
  try {
    ...
    // Auto-sync inventory when status changes to completed
