...
exports.createSalesReturn = async (req, res) => {
  try {
    ...
    for (const returnItem of validatedReturnItems) {
      ...
      if (saleItem) {
        saleItem.returnedQuantity = (saleItem.returnedQuantity || 0) + returnItem.quantityReturned;
        ...
        console.log('  ✅ Updated item:', {
          name: saleItem.name,
          returnedQty: returnItem.quantityReturned,
    ...
    } else if (originalSale.received >= originalSale.grandTotal) {
      originalSale.status = 'paid';
    } else if (originalSale.received > 0) {
    ...
    if (updateInventory && !salesReturn.inventoryUpdated) {
      for (const returnItem of salesReturn.returnItems) {
        ...
        if (inventoryItem) {
          ...
          inventoryItem.currentStock += returnItem.quantityReturned;
          
          // Update sales statistics
          inventoryItem.totalQuantitySold = Math.max(
            0, 
            inventoryItem.totalQuantitySold - returnItem.quantityReturned
          );
        ...
        for (const returnItem of salesReturn.returnItems) {
          ...
          if (saleItem) {
            saleItem.returnedQuantity = Math.max(
              0,
              (saleItem.returnedQuantity || 0) - returnItem.quantityReturned
            );
